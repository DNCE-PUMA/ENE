...
//				WRAP_CONTENT, 50).callback("onFuncionVAUSENTE");
		...
		txtPAGWEB = new TextField(this.getActivity(), false).size(
				altoComponente, 440);
		txtPAGWEB.addTextChangedListener(twFuncionBloqueo);
...
//		} else {
			...
			try {				
				Integer codigo = Integer.parseInt(bean.ciiu_cd.toString().substring(0, 2));
	...
	//	if (!chbRES_VERIF.isChecked()) {
			...
			if (!Util.esVacio(txtPAGWEB) &&  !txtPAGWEB.getText().toString().contains("WWW")) {
				mensaje = "INGRESE PAGINA WEB CORRECTA, DEBE INICIAR EN WWW";
				view = txtPAGWEB;
				return !(error = true);
			}
...
//			VERIFICACIÓN	:	Si CIIU_CD <> CIIU_V4(MARCO)  OR CIIU_S1_CD <> CIIU_V4 OR CIIU_S2_CD <> CIIU_V4 OR CIIU_S3_CD <> CIIU_V4 OR CIIU_S4_CD <> CIIU_V4 
//			Verificar 	MENSAJE	:	“Verificar: actividad económica principal o secundaria que declara en la caratula no es la misma actividad económica del marco”  
		
			String cd0x = Util.esVacio(txtCIIU_CD) ? "" : Util.getText(txtCIIU_CD.getText());
			String cd1x = Util.esVacio(txtCIIU_S1_CD) ? "" : Util.getText(txtCIIU_S1_CD.getText());
			String cd2x = Util.esVacio(txtCIIU_S2_CD) ? "" : Util.getText(txtCIIU_S2_CD.getText());
			String cd3x = Util.esVacio(txtCIIU_S3_CD) ? "" : Util.getText(txtCIIU_S3_CD.getText());
			String cd4x = Util.esVacio(txtCIIU_S4_CD) ? "" : Util.getText(txtCIIU_S4_CD.getText());
			
					
			
			if ( !cd0x.equals(App.getInstance().getMarco().ciiu_v4) && 
					!cd1x.equals(App.getInstance().getMarco().ciiu_v4) && 
					!cd2x.equals(App.getInstance().getMarco().ciiu_v4) && 
					!cd3x.equals(App.getInstance().getMarco().ciiu_v4) &&
					!cd4x.equals(App.getInstance().getMarco().ciiu_v4) )  {
	     		ToastMessage.msgBox(this.getActivity(),
					"Verificar: actividad económica principal o secundaria que declara en la caratula no es la misma actividad económica del marco", ToastMessage.MESSAGE_INFO,
					ToastMessage.DURATION_LONG);
	         }
	...
	private void inicio() {
		Log.e ("txtCIIU_CD", txtCIIU_CD.getText().toString() + "");
		Log.e ("App.getInstance().getMarco().ciiu_v4.toString()", App.getInstance().getMarco().ciiu_v4.toString() + "");
		...
		if (chbTELFIJO_NO.isChecked()) {
			Util.cleanAndLockView(getActivity(), txtTELFIJO);
		} else {
			Util.lockView(getActivity(), false, txtTELFIJO);
		}

		if (chbCEL_NO.isChecked()) {
			Util.cleanAndLockView(getActivity(), txtCEL);
		} else {
			Util.lockView(getActivity(), false, txtCEL);
		}

		if (chbEMAIL_NO.isChecked()) {
			Util.cleanAndLockView(getActivity(), txtEMAIL);
		} else {
			Util.lockView(getActivity(), false, txtEMAIL);
		}

		if (chbPAGWEB_NO.isChecked()) {
			Util.cleanAndLockView(getActivity(), txtPAGWEB);
		} else {
			Util.lockView(getActivity(), false, txtPAGWEB);
		}
...
//		txtCIIU_DESC_C.setText("");
	...
	public void FuncionP19A() {
		Integer valor = Integer.parseInt(rgCIIU_CD_A.getTagSelected("0").toString());
		if (valor == 1) {
			Util.cleanAndLockView(getActivity(), txtCIIU_DESC_B,txtCIIU_DESC_C,  imgCP19A,
					imgEP9A);
			rgCIIU_C.requestFocus();	
		} else {
			Util.lockView(getActivity(), false, txtCIIU_DESC_B, txtCIIU_DESC_C, imgCP19A,
					imgEP9A);
			txtCIIU_DESC_B.readOnly();
...
//			if (txtPAGWEB.length()>0) {
//				txtPAGWEB.setText("WWW" + txtPAGWEB.getText());
//			}
