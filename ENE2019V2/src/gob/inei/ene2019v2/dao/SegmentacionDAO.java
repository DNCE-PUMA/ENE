...
//		.append("FROM (").append(MarcoDAO.querySelect(null,null,null,null,null,usuarioId,null)).append(") Seg UNION ALL").append(" ")
	...
	public List<Periodo> getPeriodos(String codSedeOp, String equipo, String ruta) {
		...
		List<Periodo> periodos = new ArrayList<Periodo>();
		StringBuilder query = new StringBuilder();
		if(Util.getInt(App.getInstance().getUsuario().cargo_id) != App.ENCUESTADOR){
			query.append("SELECT ").append(Periodo.TODOS).append(" AS PERIODO , '--- TODOS ---' AS PERIODO_DESC UNION").append(" ");
		}
		query.append("SELECT DISTINCT S.PERIODO AS PERIODO, 'PERIODO N\u00B0 '||CAST(S.PERIODO AS VARCHAR) AS PERIODO_DESC").append(" ")
			.append("FROM ").append(TABLA).append(" S ")
			.append("INNER JOIN T_USUARIO U ON U.COD_SEDE_OPERATIVA=s.COD_SEDE_OPERATIVA ").append(" ");
		if(!Equipo.TODOS.equals(equipo)){
			query.append("AND U.EQUIPO=s.EQUIPO AND U.EQUIPO= '").append(equipo).append("' ");
		}
		if(!Ruta.TODOS.equals(ruta)){
			query.append("AND U.RUTA=S.RUTA AND U.RUTA= '").append(ruta).append("' ");
		}
		query.append("WHERE s.COD_SEDE_OPERATIVA=?").append(" ")
			.append("ORDER BY S.PERIODO ASC").append(" ");
		
		cursor = dbr.rawQuery(query.toString(), new String[]{codSedeOp,});		
        while(cursor.moveToNext()){
        	Periodo p = new Periodo();
			p.periodo = getInt("PERIODO");
			p.periodo_desc = getString("PERIODO_DESC");
...
//		String[] camposSedeOperativa = new String[]{"cod_sede", "sede"};
//		String[] camposSedeOperativaUbigeo = new String[]{"cod_sede", "ubigeo"};
//		boolean flag = super.saveOrUpdate(dbTX, TABLA_SEDES_OPERATIVA, "COD_SEDE=?", bean, new String[]{"cod_sede"}, -1, -1, camposSedeOperativa);
//		if (flag) {
//			flag = super.saveOrUpdate(dbTX, TABLA_SEDES_OPERATIVA_UBIGEO, "COD_SEDE=? AND UBIGEO=?", bean, new String[]{"cod_sede","ubigeo"}, -1, -1, camposSedeOperativaUbigeo);
//		}
//		return flag;
//	}
	...
	public boolean saveOrUpdate(Segmentacion bean, SQLiteDatabase dbTX) throws SQLException {
		...
		if (bean.id == null) { 
			...
			boolean flag1 = true;
			flag1 = existeRegistro(dbTX, "ID", TABLA, "CCDD=? AND CCPP=? AND CCDI=? AND PERIODO=?", 
						bean.ccdd, bean.ccpp, bean.ccdi, bean.periodo.toString());
			if (flag1) {
				return true;
			} else {
				Integer nextID = super.nextID(dbTX, "ID", TABLA, null); 
	...
	public static class Sede extends Entity implements Spinneable{
		public static final String NINGUNA="00";
		public static final String TODOS="99";
		...
		public String getFullSede() {
			return codigoSede + "[" + codigoSede + "-" + Util.getText(codigoSede,"") + "]";
		}
		...
		public String getSpinnerLabel() {
			if (TODOS.equals(codigoSede) || NINGUNA.equals(codigoSede)) {
				return sede;
			}
			return codigoSede + " : " + sede;
		}
	}
	...
	public List<Sede> getSedes(String codigoSedeOperativa) {
		...
		SQLiteDatabase dbr = dbh.getReadableDatabase();
		StringBuilder query = new StringBuilder();
		query.append("SELECT DISTINCT COD_SEDE, SEDE").append(" ")
			.append("FROM ").append(TABLA_SEDES_OPERATIVA).append(" ")
			.append("WHERE COD_SEDE = ?").append(" ");
		query.append("ORDER BY COD_SEDE").append(" ");
		cursor = dbr.rawQuery(query.toString(), new String[]{codigoSedeOperativa});		
		while(cursor.moveToNext()){
			Sede p = new Sede();
			p.codigoSede = getString("COD_SEDE");
			p.sede = getString("SEDE");
			sedes.add(p);
		}		
		cursor.close();
		cursor = null;
	...
	public static class Equipo extends Entity implements Spinneable{
		...
		public String getFullSede() {
			return codigo + "[" + codigo + "-" + Util.getText(codigo,"") + "]";
		}
	...
	public List<Equipo> getEquipos(String codigoSedeOperativa) {
		List<Equipo> sedes = new ArrayList<Equipo>();
		SQLiteDatabase dbr = dbh.getReadableDatabase();
		StringBuilder query = new StringBuilder();
		query.append("SELECT DISTINCT '").append(Equipo.TODOS).append("' AS EQUIPO UNION").append(" ")
			.append("SELECT DISTINCT EQUIPO").append(" ")
			.append("FROM ").append(TABLA).append(" ")
			.append("WHERE COD_SEDE_OPERATIVA = ?").append(" ");
		query.append("ORDER BY EQUIPO").append(" ");
		cursor = dbr.rawQuery(query.toString(), new String[]{codigoSedeOperativa});		
		while(cursor.moveToNext()){
			Equipo p = new Equipo();
			p.codigo = getString("EQUIPO");
			sedes.add(p);
		}		
		cursor.close();
		cursor = null;
	...
	public static class Periodo extends Entity implements Spinneable{
		public static final Integer NINGUNA=-1;
		public static final Integer TODOS=999;
		private static final long serialVersionUID = 1L;
	...
	public static class Ruta extends Entity implements Spinneable {
		public static final String NINGUNA="000";
		public static final String TODOS="999";
		private static final long serialVersionUID = 1L;
		public String codigoSede;
		...
		@Override
		public String getSpinnerLabel() {
			if (TODOS.equals(rutaCodigo)) {
				return "-- TODOS --";
			}
			if (NINGUNA.equals(rutaCodigo)) {
				return ruta;
			}
			return "RUTA " + ruta;
		}
	...
	public List<Ruta> getRutas(String codigoSedeOperativa, String equipo) {
		...
		SQLiteDatabase dbr = dbh.getReadableDatabase();
		StringBuilder query = new StringBuilder();
		query.append("SELECT NULL AS COD_SEDE_OPERATIVA, NULL AS EQUIPO, '").append(Ruta.TODOS).append("' AS RUTA UNION").append(" ")
			.append("SELECT DISTINCT COD_SEDE_OPERATIVA, EQUIPO, RUTA").append(" ")
			.append("FROM ").append(TABLA).append(" ")
