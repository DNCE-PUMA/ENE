...
public class Mod_IV_Fragment_012 extends FragmentForm {
	...
	public RadioGroupOtherField rgC4P440A;
	...
	public RadioGroupOtherField rgC4P4411;
	...
	protected void buildFields() {
		...
		rgC4P440A = new RadioGroupOtherField(this.getActivity(),
				R.string.c4R_solidos,
				R.string.c4R_liquido,
				R.string.c4ambas).size(150, 250)
				.orientation(ORIENTATION.VERTICAL).callback("FuncionP440A");
...
//				.callback("FuncionP440A");
		...
		rgC4P4411 = new RadioGroupOtherField(this.getActivity(),
				R.string.c1c100m4p441_1a, R.string.c1c100m4p0441_2).size(
				WRAP_CONTENT, 600).orientation(ORIENTATION.VERTICAL).callback("FuncionP4411");
		...
		lbl40b1 = new LabelComponent(this.getActivity(), App.ESTILO)
				.size(altoComponente + 20, 350).text("TRATAMIENTO/GESTIÓN")
				.textSize(17).centrar();
		lbl40b2 = new LabelComponent(this.getActivity(), App.ESTILO)
				.size(altoComponente + 20, 150).text("RESIDUOS SÓLIDOS")
				.textSize(17).centrar();
		lbl40b3 = new LabelComponent(this.getActivity(), App.ESTILO)
				.size(altoComponente + 20, 150).text("RESIDUOS LÍQUIDOS")
				.textSize(17).centrar();
...
//		grP40.colorFondo(R.color.transparent);
		...
		Filtros.setFiltro(txtC4P441B, Filtros.TIPO.NUMBER, 9, -1, null, 1,999999998,999999999);
	}
	...
	protected View createUI() {
		...
		q1 = createQuestionSection(R.string.c1c100m4p440a, rgC4P440A);
		...
		q4 = createQuestionSection(R.string.c1c100m4p4411, rgC4P4411);
		q5 = createQuestionSection(R.string.c1c100m4p441b, txtC4P441B);
	...
	private boolean validar() {
		...
		if (Util.esVacio(rgC4P440A)) {
			mensaje = pregunta_no_vacia.replace("$", "La pregunta 440A");
			view = rgC4P440A;
			return !(error = true);
		}
...
//			if (rgC4P440A.isTagSelected(2)) {
//				if (Util.esVacio(x)) {
//					mensaje = pregunta_no_vacia.replace("$",
//							"La pregunta P440B - Residuos Líquidos.");
//					view = x;
//					return !(error = true);
//				}
//			}
//		}
		if(rgC4P440A.isTagSelected(1)){
			...
			if (rgC4P440B_A_3.isTagSelected(2)
					...
					mensaje = "Error: Debe existir información positiva en alguna de las alternativas";
					view = rgC4P440B_A_3;
					error = true;
					return false;
				}
	...
	if(rgC4P440A.isTagSelected(3)){
		...
		if (rgC4P440B_B_1.isTagSelected(2)
			...
			Integer valor1 = Integer.parseInt(rgC4P4411.getTagSelected("0").toString());
			if (valor1 == 1) {
			...
			if(txtC4P441B.getText().length()>0 && Util.getInt(txtC4P441B.getText().toString())==0){
				mensaje = "Error: El valor del gasto en la gestión de residuos sólidos debe ser mayor a 0";
				view = txtC4P441B;
				error = true;
				return false;
	...
	private void inicio() {
		FuncionP440A();
		FuncionP4411();
		rgC4P440A.requestFocus();
	}

	public void FuncionP440A() {
		Integer valor1 = Integer.parseInt(rgC4P440A.getTagSelected("0").toString());

		if (valor1 == 1) {
			Util.cleanAndLockView(getActivity(), rgC4P440B_B_1, rgC4P440B_B_2,
					rgC4P440B_B_3, rgC4P440B_B_4, rgC4P440B_B_5, rgC4P440B_B_6);
		} else{
			Util.lockView(getActivity(), false, rgC4P440B_B_1, rgC4P440B_B_2,
					rgC4P440B_B_3, rgC4P440B_B_4, rgC4P440B_B_5, rgC4P440B_B_6);
		}
		if ( valor1 == 2) {
	...
	public void FuncionP4411() {
		Integer valor1 = Integer.parseInt(rgC4P4411.getTagSelected("0").toString());

		if (valor1 == 2) {
			Util.cleanAndLockView(getActivity(), txtC4P441B);
		} else{
			Util.lockView(getActivity(), false, txtC4P441B);
		}
	}
