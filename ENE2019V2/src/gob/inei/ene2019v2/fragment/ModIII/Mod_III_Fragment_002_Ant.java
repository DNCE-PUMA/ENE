...
//				.orientation(ORIENTATION.HORIZONTAL).centrar();
		...
		LabelComponent lblc = new LabelComponent(this.getActivity(),
				App.ESTILO).size(altoComponente, 250).text("¿Cuántas horas de trabajo fueron…?")
				.textSize(15).negrita().centrar();
		...
		LabelComponent lblNHoras1 = new LabelComponent(this.getActivity(),
				App.ESTILO).size(altoComponente, 250).text("Número de horas")
				.textSize(15).negrita().alinearIzquierda();
...
//		txtC3P302C_1.addTextChangedListener(new twFuncionBloqueo1(txtC3P302A_1));			
	...
	private boolean validar() {
		...
		// VERIFICACIÓN 1
//VERIFICACIÓN	:	  Si C3P302A_A2=1 and P25<>1  Verificar
//                Si C3P302A_A3=1 and P25<>2   Verificar
//   MENSAJE	:	“Error: Régimen de contrato laboral no se relaciona con P25“
		...
		if (error1 && !avance1) {
			...
			DialogComponent dlg = new DialogComponent(
					...
					"Verificar: Régimen de contrato laboral no se relaciona con P25“");
			dlg.showDialog();
			return false;
		}
...
//		}		
		...
		if (Util.getInt(bean.c3p301_omi )==1  &&  Util.getInt(bean.c3p302_omi )==1){			
			if ( Util.getInt(Total302) != SumGeneA) {
				//mensaje = "Error: No concuerda total de trabajadores según categoría ocupacional con total de trabajadores según nivel  educativo";
				ToastMessage.msgBox(this.getActivity(), "Verificar: No concuerda total de trabajadores según categoría ocupacional con total de trabajadores según nivel  educativo",
						ToastMessage.MESSAGE_INFO, ToastMessage.DURATION_LONG);	
			//	view = txtC3P302A_8;
			//	return !(error = true);	
			}	
...
//			}	
			...
			//	mensaje = "Error: No concuerda total de trabajadores (Mujeres) según categoría ocupacional con total de trabajadores (Mujeres) según nivel educativo";
				ToastMessage.msgBox(this.getActivity(), "Verificar: No concuerda total de trabajadores (Mujeres) según categoría ocupacional con total de trabajadores (Mujeres) según nivel educativo",
						ToastMessage.MESSAGE_INFO, ToastMessage.DURATION_LONG);	
			//	view = txtC3P302B_8;
			//	return !(error = true);	
			}	
			...
			//	mensaje = "Error: No concuerda total de trabajadores (Hombres) según categoría ocupacional con total de trabajadores (Hombres) según nivel educativo";
				ToastMessage.msgBox(this.getActivity(), "Verificar: No concuerda total de trabajadores (Hombres) según categoría ocupacional con total de trabajadores (Hombres) según nivel educativo",
						ToastMessage.MESSAGE_INFO, ToastMessage.DURATION_LONG);	
			//	view = txtC3P302C_8;
			//	return !(error = true);	
			}	
...
//		}
		...
		if (rgC3P303.isTagSelected(1) ) {
			if (Util.getInt(txtC3P303_A_1) >19 ){
				mensaje = "Error: Un turno de trabajo con 20 a más horas laborales";
				view = txtC3P303_A_1;
				error = true;
				return false;
			}			
		...
		if (rgC3P303.isTagSelected(1) ) {
			if (Util.getInt(txtC3P303_A_2) >15 ){
				mensaje = "Error: Un turno de trabajo con 15 a más horas laborales";
				view = txtC3P303_A_1;
				error = true;
				return false;
			}			
		...
		if (rgC3P303.isTagSelected(1) ) {
			if (Util.getInt(txtC3P303_A_3) >10 ){
				mensaje = "Error: Un turno de trabajo con 10 a más horas laborales";
				view = txtC3P303_A_1;
				error = true;
				return false;
			}			
...
//		VERIFICACIÓN 2	:	Si C3P303 =1 and C3P303A>10   Verificar
...
//		VERIFICACIÓN	:	Si C3P303 =1 and C3P303A>18  Error
...
//			mensaje = "Verificar cantidad de trabajadores por nivel educativo menor a total por ocupación";
//			view = txtC3P302B_8;
//		//	return !(error = true);
//		}
