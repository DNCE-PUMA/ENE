...
public class Visita_Fragment_001 extends FragmentForm implements Respondible {
	...
	public static final int OTRO = 9;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		listening();
	//	executeEvents();
	//	campos =  new String[] {"ENC_DNI", "ENC_NOM"} ;
    ...
    private void executeEvents() {
		txtENC_DNI.addTextChangedListener(new BlockWatcher());
	}
	...
	private boolean validaInfoEncuestador() {
		...
		if (Util.esVacio(txtENC_DNI)) {
			mensaje = "DNI del Operador/a de Empresas no puede estar vacio.";
			view = txtENC_DNI;
			return !(error = true);
		}
		if (Util.esVacio(txtENC_NOM)) {
			mensaje = "Nombre del Operador/a de Empresas no puede estar vacio.";
			view = txtENC_NOM;
			return !(error = true);
		}
	...
	private class BlockWatcher implements TextWatcher {
//		private View view;
...
//			if(s.length()==8){
//			Caratula	caratula = getCuestionarioService().getEmpadronador( App.getInstance().getEmpresa().id,  s.toString(),campos);
//	    		if(caratula!=null){
//	    			txtENC_NOM.setText(caratula.enc_nom);
//
//	    		}
//	    	} else {
//	    		txtENC_NOM.setText("");
