...
public class Mod_IV_Fragment_023_01 extends DialogFragmentComponent {
	...
	protected void buildFields() {
		...
		// CONTROL DE FLECHAS
//		btnNext = new ImageComponent(getActivity(), R.drawable.arriba,R.drawable.arriba);
...
//		});
	...
	private boolean validar() {
		...
		if (!Util.esDiferente(detalle.m4p076, 1)) {
			...
			if (!Util.esDiferente(detalle.m4p077, 1)) {
				...
				if (Util.getInt(txtM4P080) < Util.getInt(bean2.funcionamiento)) {
					mensaje = "Error: Año de implementación debe ser MAYOR al Año de FUNCIONAMIENTO";
					view = txtM4P080;
					error = true;
					return false;
				}
		...
		if (contador >= 6) {
			mensaje = "Si cuenta con procesos debe seleccionar la opción “SI” en uno de ellos";
			view = rgM4P076;
			error = true;
			return false;
