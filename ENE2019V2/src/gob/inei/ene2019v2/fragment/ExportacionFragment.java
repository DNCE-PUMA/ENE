...
import gob.inei.ene2019v2.controller.EnvioServidor;
import gob.inei.ene2019v2.controller.Exportacion;
...
//		LinearLayout q5 = createQuestionSection(q1);
	...
	protected void buildFields() {
		...
		lblSel = new LabelComponent(getActivity()).text(R.string.c1_selall).size(WRAP_CONTENT, 550).negrita();
		...
		tcMarco = new TableComponent(getActivity(), this,
				App.ESTILO).size(600, 700)
				.dataColumHeight(65).headerHeight(60).headerTextSize(15);
		tcMarco.addHeader(R.string.m_seleccionado, 0.3f, CheckBoxField.class);

		tcMarco.addHeader(R.string.m_tc_exp_cod, 0.6f);
		tcMarco.addHeader(R.string.m_tc_exp_nom, 1.7f, ALIGN.LEFT);
	...
	private class RowExportar implements IDetailEntityComponent {
		...
		public boolean getResFin() {
			return registro.getUbigeo()!=null;
		}
	...
	public void cargarMarco(Sede sede, Equipo equipo, Ruta ruta, Periodo periodo){
		if (sede != null && equipo != null && ruta != null && periodo != null) {
			periodo = periodo;
			exportables  = getMarcoService().getMarco(sede.codigoSede, equipo.codigo, 
					ruta.ruta, periodo.periodo, FragmentForm.OPCION.ONE);
	...
	public void onSeleccionadoChangeValue(Object entity, Integer row, Integer opcion) {
		if (entity != null) {
			...
			if(opcion == 1 && !tmp.getResFin()){
				ToastMessage.msgBox(this.getActivity(), "Empresa no tiene Resultado Final.",
